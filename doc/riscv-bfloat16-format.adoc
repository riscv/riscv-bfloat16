...
It is important to note that tininess detected after rounding is performed as if the exponent were unbounded.
This means that the result needs to be normalized before rounding. This is different from the actual rounding
on the final result which includes first denormalizing the result until the exponent is the lowest supported by
the format. This can result in underflow being signalled for values that are rounded back to the normal range.
